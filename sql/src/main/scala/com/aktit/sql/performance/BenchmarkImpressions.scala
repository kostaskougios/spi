			...
			"\n" +
				Tabulator.format(
					Seq(
						...
						q.measureDateQuery("2010-02-01T00:00:00.00Z", "2010-03-01T00:00:00.00Z", (t, start, end) => s"select count(distinct userId) from impressions_$t where date between '$start' and '$end'"),
						q.measureDateQuery("2010-08-01T00:00:00.00Z", "2010-09-01T00:00:00.00Z", (t, start, end) => s"select count(distinct userId) from impressions_$t where date between '$start' and '$end'")
					)
				)
		)
