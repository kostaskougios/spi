...
object BenchmarkOrders extends Logging {
  def main(args: Array[String]): Unit = {
    ...
    spark.read.format("avro").load(s"$srcDir/avro").createOrReplaceTempView("orders_avro")
        ...
        Tabulator.format(
          Seq(
            ...
            q.measureDateQuery(
              ...
              (t, start, end) =>
                s"select productCode,productTitle,max(discountPercentageApplied) d from orders_$t where date between '$start' and '$end' group by productCode,productTitle order by d desc limit 5"
            ),
            q.measureDateQuery(
              "2010-03-01T00:00:00.00Z",
              "2010-06-01T00:00:00.00Z",
              (t, start, end) =>
                s"select productCode,productTitle,max(discountPercentageApplied) d from orders_$t where date between '$start' and '$end' group by productCode,productTitle order by d desc limit 5"
            ),
            q.measureQuery(t =>
              s"select productCode,productTitle,max(discountPercentageApplied) d from orders_$t group by productCode,productTitle order by d desc limit 5"
            ),
