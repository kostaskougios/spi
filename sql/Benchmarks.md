...
2nd run:
    ...
    |Query                                                                                                                                                                                                              |  Avro|Parquet|   ORC|
    +-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+------+-------+------+
    |                                                                                                                            select sum(boughtPrice) s,userId from orders_* group by userId order by s desc limit 10|801061| 181324|169441|
    |select productCode,productTitle,max(discountPercentageApplied) d from orders_* where date between '2010-01-01T00:00:00.00Z' and '2010-03-01T00:00:00.00Z' group by productCode,productTitle order by d desc limit 5|732058|1012651|990737|
    |select productCode,productTitle,max(discountPercentageApplied) d from orders_* where date between '2010-03-01T00:00:00.00Z' and '2010-06-01T00:00:00.00Z' group by productCode,productTitle order by d desc limit 5|740687|1014109|977440|
    |                                                                           select productCode,productTitle,max(discountPercentageApplied) d from orders_* group by productCode,productTitle order by d desc limit 5|753989| 119432|119096|
    |                                                                                                                                                                         select * from orders_* where userId=500000| 25248|  24881|  7274|
    |                                                                                                                                                                          select * from orders_* order by productId|761421| 623605|663496|
    +-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+------+-------+------+
