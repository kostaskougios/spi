...
class PagesToDatabaseJobTest extends AbstractSparkSuite {
  ...
  test("preparePageTable") {
    preparePageTable(sc.parallelize(Seq(Page1a, Page2))).collect().toSet should be(
      Set(
  ...
  test("prepareContributorsTable") {
    prepareContributorsTable(sc.parallelize(Seq(Page1a, Page2))).collect().toSet should be(
      Set(
        (100, "kostas"),
        (102, "nick")
      )
  ...
  test("prepareRevisionsTable") {
    ...
    prepareRevisionsTable(sc.parallelize(Seq(Page1a, Page2))).collect().toSet should be(
      Set(
  ...
  test("populatePagesTable") {
    ...
    prepareTable(
      TestTable,
      SQL(
        "create table " + TestTable + "(id bigint not null,lang varchar not null,title varchar, redirect varchar, constraint PK_PAGETEST primary key (id,lang))"
      )
    ) {
  ...
  test("populateRevisionsTable") {
    ...
    prepareTable(
      TestTable,
      SQL(
        "create table " + TestTable + "(id bigint primary key,PARENTID bigint,TIME timestamp,COMMENT varchar,MODEL varchar,FORMAT varchar,TEXT varchar,SHA1 varchar)"
      )
    ) {
      val data = sc.parallelize(Seq(Page1a, Page2))
      ...
      val loaded = sqlContext.read
        ...
        .map { row =>
          (
            row.getLong(0),
            row.getLong(1),
            new DateTime(row.getAs[Timestamp](2)),
            row.getString(3),
            row.getString(4),
            row.getString(5),
            row.getString(6),
