...
object Boundaries {
  ...
  def fromEdges(universe: Universe, edges: Seq[Edge]): Boundaries = {
    ...
    val bottomBoundary = edges
      .collectFirst { case e: TopSide =>
        e.allLive
      }
      .getOrElse(Seq.empty)
    ...
    val topBoundary = edges
      .collectFirst { case e: BottomSide =>
        e.allLive
      }
      .getOrElse(Seq.empty)

    val left = edges
      .collectFirst { case e: RightSide =>
        e.allLive
      }
      .getOrElse(Seq.empty)

    val right = edges
      .collectFirst { case e: LeftSide =>
        e.allLive
      }
      .getOrElse(Seq.empty)
  ...
  def apply(
    ...
    BitSetBoundaries(
      width,
      height,
      BitSet(top.map(_ + 1).toIndexedSeq: _*),
      BitSet(bottom.map(_ + 1).toIndexedSeq: _*),
      BitSet(left.toIndexedSeq: _*),
      BitSet(right.toIndexedSeq: _*)
    )
  }

  private def check(a: Array[Int], min: Int, max: Int) = for (i <- a)
    if (i < min || i > max) throw new IllegalArgumentException(s"coordinate out of bounds: $i should be between $min and $max")
