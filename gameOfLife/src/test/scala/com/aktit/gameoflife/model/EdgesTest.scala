...
class EdgesTest extends FunSuite
{
	...
	test("partitions edges, top side") {
		...
		edges(topSide = side).sendToNeighbors.collect {
	...
	test("partitions edges, bottom side") {
		...
		edges(bottomSide = side).sendToNeighbors.collect {
	...
	test("partitions edges, left side") {
		...
		edges(leftSide = side).sendToNeighbors.collect {
	...
	test("partitions edges, right side") {
		...
		edges(rightSide = side).sendToNeighbors.collect {
