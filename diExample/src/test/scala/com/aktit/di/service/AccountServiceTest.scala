...
/** Here we need to test if everything is wired correctly. We don't have to test the transfer business logic since that is taken care by AccountTest.
  *
  * @author
  *   kostas.kougios 14/05/19 - 08:17
  */
class AccountServiceTest extends AbstractDiSuite {
  ...
  test("executeTransfers end to end") {
    new App {
      ...
      accountDao.read.toSet should be(
        Set(
          acc,
          account(name = "acc1", amount = 6, lastUpdated = transferTime)
        )
